...
class App extends Component {
  ...
  filterThumbNails = () => {
    const {activeTabBtn} = this.state
    return imagesList.filter(eachItem => eachItem.category === activeTabBtn)
  }

  onChangeTab = tabId => {
    this.setState({activeTabBtn: tabId})
  }

  onClickImage = id => {
    const {uniqueImg} = this.state
    const newUniqueImg = imagesList[Math.floor(Math.random() * 30)]

    if (uniqueImg.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        uniqueImg: newUniqueImg,
      }))
    } else {
      clearInterval(this.intervalId)
      this.setState({isGameOver: true})
  ...
  onResetGame = () => {
    this.setState({
      ...
      score: 0,
      time: 60,
      uniqueImg: imagesList[Math.floor(Math.random() * 30)],
    })
    this.componentDidMount()
  }

  render() {
    const {score, time, activeTabBtn, uniqueImg, isGameOver} = this.state
    const {imageUrl} = uniqueImg
    const filteredThumbNails = this.filterThumbNails()
              ...
              <div className="score-cont">
                ...
                <div className="timer-cont">
                  <img
                    ...
                    alt="timer"
                  />
                  <p className="score-and-time">{`${time} sec`}</p>
                </div>
              </div>
            </li>
          </ul>
        ...
        <div className="bg-cont">
          {isGameOver === false && (
            <div className="container">
              <img className="match-img" src={imageUrl} alt="match" />
